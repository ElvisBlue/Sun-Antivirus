...
'Fixed bugs by Elvis for Sun AntiVirus
...
lRegResult = RegOpenKeyEx(hKey, strPath, 0&, KEY_READ, hCurKey)
...
Do
  ...
  lRegResult = RegEnumValue(hCurKey, lCounter, strValueName, lValueNameSize, ByVal 0&, REG_DWORD, byDataBuffer(0), lDataBufferSize) 'byval byDataBuffer(0) mean NULL
  ...
  Else
    If lCounter = 0 Then Exit Function
    Exit Do
  End If
Loop
